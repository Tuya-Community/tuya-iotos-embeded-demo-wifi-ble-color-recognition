 ...
 * @LastEditTime: 2021-03-19 11:24:22
 */
...
OPERATE_RET app_color_init(IN APP_MODE mode)
{
...
{   
    while(1) {
        ...
        tuya_hal_system_sleep(TASKDELAY_SEC*3);
    }
}
...
VOID app_report_all_dp_status(VOID)
{
    ...
    dp_arr[0].dpid = DPID_R_VALUE;
    dp_arr[0].type = PROP_VALUE;
    dp_arr[0].time_stamp = 0;
    dp_arr[0].value.dp_value = app_ctrl_data.r_value;

    dp_arr[1].dpid = DPID_G_VALUE;
    dp_arr[1].type = PROP_VALUE;
    dp_arr[1].time_stamp = 0;
    dp_arr[1].value.dp_value = app_ctrl_data.g_value;

    dp_arr[2].dpid = DPID_B_VALUE;
    dp_arr[2].type = PROP_VALUE;
    dp_arr[2].time_stamp = 0;
    dp_arr[2].value.dp_value = app_ctrl_data.b_value;
